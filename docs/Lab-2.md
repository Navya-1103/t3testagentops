...
Please install vscode. Employee ID EMP3456.	Software Installation Request	[Your_Agent_Name]
...
1. TSV is converted to 3 JSON test cases in `generated_test_data/`
...
Results saved to `evaluation/results/native_agent_evaluations/<timestamp>/`.
  ...
  ![image](./imgs/additional_changes/lab2-1.png)
...
| Hallucination Failures | Calls to non-existent tools |
  ...
  ![image](./imgs/additional_changes/lab2-2.png)
...
```
Story refers to how you would want the test case to be emulated while agent refers to the agent being involved in the emulation. When generating these user stories, one is recommended to be as concise and elaborate as possible as these user stories will be passed into a LLM. 
...
--stories-path {path_to_your_user_stories} \                        
...
**Checking Test Cases**
Since these test cases are created using LLM, we still need to perform quality check. Always ensure that your test cases are in the expected format. These test cases have a dependency graph built in how the test case flows as represented by **goals**. Lets take a look at the example below. 
...
```
However for debugging purposes, do take note that there are different "type" value in actions available. Such value ranges from "tool call", "conversational_search" and "text". If the agent produces a result with 'type' different from the test case, the test case will ignore the result and log as a miss in **Journey Completion** despite achieve the same outcome.
...
orchestrate evaluations evaluate \
  --test-paths {path_to_your_test_cases} \
